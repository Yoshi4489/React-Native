    ...
    <SafeAreaView>
      {Platform.OS === "android" ? (
        <View style={styles.backBtn}>
          <Button title="Back" onPress={handleClick}></Button>
        </View>
      ) : (
      ...
      {theory.map((items) => {
        return (
          <View>
            ...
            <Text style={{fontSize: 30, marginBottom: 10}}>{selectedTheory.toString().split(' ')}</Text>
            {calculate ? (
              <View style={{flexDirection: 'column', justifyContent: 'center'}}>
                {items.all.map((item, index) => {
                  return (
                    <View key={Math.random() * index} style={{flexDirection: 'row', alignItems: 'center', marginBottom: 10}}>
                      <Text key={Math.random() / 3} style={{marginRight: 20, fontSize: 20}}>{item}</Text>
                      <TextInput
                        ...
                        style={{borderWidth: 2, borderColor: 'black', padding: 20, width: '80%', borderRadius: 5}}
                      />
                    </View>
                  );
                })}
